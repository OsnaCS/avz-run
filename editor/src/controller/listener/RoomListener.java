...
public class RoomListener extends RectangleListener {
	...
	public LinkedList<Way> getAllways() {
		return allways;
	}

	public void setAllways(LinkedList<Way> allways) {
		this.allways = allways;
	}

	public RoomListener(DrawableObjectProcessing delegate, Room room, LinkedList<Way> allways) {
		super(delegate);
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (tmp == null) {
			...
			if (SwingUtilities.isRightMouseButton(e)) {
				room.spin();
				updateDelegate(new Point(e.getX() + room.getA().x, e.getY() + room.getA().y),
	...
	public void mouseMoved(MouseEvent e) {
		...
		if (tmp == null) {
			...
			// determines position of rectangle with position of cursor and
	...
	private void updateDelegate(Point ua, Point ue) {
		...
		DashedRoom r = new DashedRoom(room.getName(), ua.x, ua.y, ue.x, ue.y, room.getWaylist());
